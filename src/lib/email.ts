...
const TEMPLATE_NEW_ORDER_ADMIN = 'YOUR_ADMIN_NEW_ORDER_TEMPLATE_ID';
const TEMPLATE_NEW_ORDER_CUSTOMER = 'YOUR_CUSTOMER_NEW_ORDER_TEMPLATE_ID';
...
const sendEmail = (templateId: string, params: TemplateParams) => {
    if (!EMAILJS_PUBLIC_KEY || EMAILJS_PUBLIC_KEY === 'YOUR_PUBLIC_KEY') {
        console.error("EmailJS Public Key is not configured in src/lib/email.ts");
        return;
    }
    if (!templateId || templateId.includes('YOUR_')) {
        console.error(`EmailJS Template ID '${templateId}' is not configured in src/lib/email.ts`);
        return;
    }
...
export const triggerNewOrderEmails = (orderData: { id: string; shippingDetails: { name: string, email: string }; orderDate: Date; totalAmount: number; }) => {
    ...
    sendEmail(TEMPLATE_NEW_ORDER_ADMIN, {
        ...commonParams,
        to_email: ADMIN_EMAIL, // This is for template parameters, actual email is sent based on EmailJS settings
        to_name: 'Admin',
